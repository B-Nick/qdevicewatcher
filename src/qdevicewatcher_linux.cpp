...
{ //zDebug("%s active", qPrintable(QTime::currentTime().toString()));
    enum class ActionType {Unknown, Add, Remove, Change};

    qDebug() << "parseDeviceInfo()+" << n;
    ...
    { //buffer.canReadLine() always false?
        ...
        else if (line.startsWith("HID_UNIQ="))
        {
            appendData(line, dev);
        }
        else if (line.startsWith("DEVTYPE="))
        ...
        else if (line.startsWith("PRODUCT="))   // VID/PID/REV
        {
            appendData(line, dev);
        }
        qDebug() << "DEV:" << dev;
